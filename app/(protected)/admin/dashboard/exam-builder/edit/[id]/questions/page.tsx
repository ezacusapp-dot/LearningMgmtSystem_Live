// src/app/admin/dashboard/exam-builder/edit/[id]/questions/page.tsx
"use client";
...
const isTempId = (id?: string) => !id || id.startsWith("q_") || id.startsWith("opt_");
...
export default function EditExamQuestionsPage() {
  ...
  const examId = params?.id as string; // ← use the ROUTE param, never localStorage
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.status && res.data) {
          // Only the exam's ungrouped (non-sectioned) questions belong on this page.
          const dbQuestions = (res.data.questions || []).filter((q: any) => !q.sectionId);
          setQuestions(
            dbQuestions.length > 0
              ? dbQuestions.map((q: any, i: number) => mapDbQuestionToEditor(q, i))
              : [newQuestion(0)]
          );
        } else {
          setNotFound(true);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        setTimeout(() => router.push(`/admin/dashboard/exam-builder/edit/${examId}`), 1500);
      } else {
        showToast(response.message || "Failed to save exam");
      }
    } catch (error) {
      console.error("Save exam error:", error);
      showToast("An error occurred while saving the exam");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0a0c12] text-white font-sans">
      ...
      <div className="relative max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-start gap-4 mb-6 flex-wrap">
          <button onClick={() => router.push(`/admin/dashboard/exam-builder/edit/${examId}`)} className="p-2 rounded-lg hover:bg-white/10"><span className="text-2xl">←</span></button>
          <div className="flex-1"><h1 className="text-2xl font-extrabold text-white">Edit Questions</h1><p className="text-xs text-slate-500">Each question supports text, image, and code — options can have text + image</p></div>
