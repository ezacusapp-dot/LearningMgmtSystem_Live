...
export default function AddModal({
  ...
  const handleNameChange = (val) => {
    ...
    if (formData.username) {
      ...
      if (first) set("username", generateUsername(first, last));
    }
  };
      ...
      <div style={{ background:"#161b27", border:"1px solid #2d3448", borderRadius:16, width:"100%", maxWidth:720, maxHeight:"92vh", overflowY:"auto", display:"flex", flexDirection:"column", boxShadow:"0 24px 80px rgba(0,0,0,0.6)", animation:"slideUp 0.25s ease" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"1.3rem 1.5rem", borderBottom:"1px solid #1e2535", background:"#131720", borderRadius:"16px 16px 0 0", flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              <h2 style={{ margin:"0 0 2px", fontSize:"1.05rem", fontWeight:700, color:"#f1f5f9" }}>Add New Student</h2>
              <p style={{ margin:0, fontSize:"0.76rem", color:"#475569" }}>Register a new student profile</p>
            </div>
          </div>
        ...
        <div style={{ padding:"1.4rem 1.5rem", display:"flex", flexDirection:"column", gap:"1.1rem" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem" }}>
            ...
            <Field label="Phone" icon={<Ic.Phone />} required error={errors.phone}>
              <input type="tel" value={formData.phone} onChange={e => set("phone", e.target.value)} placeholder="10-digit number" {...inp(errors.phone)} />
            </Field>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem" }}>
            ...
            <Field label="Parent Phone" icon={<Ic.Phone />} required error={errors.parentPhone}>
              <input type="tel" value={formData.parentPhone} onChange={e => set("parentPhone", e.target.value)} placeholder="10-digit number" {...inp(errors.parentPhone)} />
            </Field>
          </div>

          <Field label="Parent Email" icon={<Ic.Mail />} error={errors.parentEmail}>
            <input type="email" value={formData.parentEmail} onChange={e => set("parentEmail", e.target.value)} placeholder="parent@email.com" {...inp(errors.parentEmail)} />
          </Field>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end", gap:10, padding:"1rem 1.5rem", borderTop:"1px solid #1e2535", background:"#131720", borderRadius:"0 0 16px 16px", flexShrink:0 }}>
          ...
          <button onClick={() => { if (validate()) onSave(); }} disabled={loading}
            ...
            {loading ? <><Ic.Loader />Saving…</> : <><Ic.Save />Add Student</>}
          </button>
        </div>
      </div>
    </div>
