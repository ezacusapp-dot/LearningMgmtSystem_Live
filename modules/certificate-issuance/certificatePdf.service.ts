...
/**
 * 🔑 Uploads the generated PDF to Vercel Blob storage instead of writing to
 * local disk. Vercel's serverless functions have a read-only filesystem
 * (except /tmp, which is ephemeral and per-instance), so writing to
 * public/certificates worked locally but silently failed / vanished in
 * production. Vercel Blob gives us a real, persistent, publicly-fetchable
 * URL that works identically in both environments.
 *
 * Requires BLOB_READ_WRITE_TOKEN to be set (Vercel sets this automatically
 * once you create a Blob store and link it to the project; for local dev,
 * copy it into your .env.local from the Vercel dashboard → Storage tab).
 */
async function uploadToBlob(certificateNumber: string, pdfBuffer: Buffer): Promise<string> {
  const blob = await put(
    `certificates/${certificateNumber}.pdf`,
    pdfBuffer,
    {
      ...
      addRandomSuffix: false, // keep deterministic filenames per certificate number
    }
  );
  return blob.url;
}
...
export async function renderCertificatePdf(
  ...
  const browser = await puppeteer.launch({
    ...
    executablePath: isLocal
      ? process.env.CHROME_PATH ||
        'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe' // adjust for your OS
  ...
  try {
    ...
    const pdfUrl = await uploadToBlob(data.certificateNumber, Buffer.from(pdfBuffer));
    return { pdfUrl };
  } finally {
    await browser.close();
  }
