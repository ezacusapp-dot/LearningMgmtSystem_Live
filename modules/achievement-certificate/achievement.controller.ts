...
//         query.percentage = parseFloat(query.percentFrom as string);
//       }
//       if (query.percentTo) {
...
export const achievementCertificateController = {
  async list(req: NextRequest) {
    try {
      ...
      const query: Record<string, string | number | undefined> = { ...rawQuery };
      
      // Handle float values in query parameters
      if (rawQuery.percentFrom) {
        query.percentFrom = parseFloat(rawQuery.percentFrom);
      }
      if (rawQuery.percentTo) {
        query.percentTo = parseFloat(rawQuery.percentTo);
      }
  ...
  async create(req: NextRequest) {
    try {
      ...
      // Create a clean body object with proper types
      const cleanBody: Record<string, any> = { ...body };
      
      // Ensure float values are parsed correctly
      if (body.percentFrom !== undefined && typeof body.percentFrom === 'string') {
        cleanBody.percentFrom = parseFloat(body.percentFrom);
      }
      if (body.percentTo !== undefined && typeof body.percentTo === 'string') {
        cleanBody.percentTo = parseFloat(body.percentTo);
      }
      
      const created = await service.create(cleanBody);
      return ok(created, 201);
    } catch (error) {
      return fail(error);
    }
  ...
  async update(req: NextRequest, id: string) {
    try {
      ...
      const updated = await service.update(parsedId, cleanBody);
      return ok(updated);
    } catch (error) {
      return fail(error);
    }
