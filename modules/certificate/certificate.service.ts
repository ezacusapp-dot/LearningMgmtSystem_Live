...
export const certificateService = {
  ...
  async generateCertificate(input: GenerateCertificateSchemaType) {
    ...
    const template = await certificateRepository.findTemplateByCourseCategory(
      course.courseCategory.id
    );
    if (!template) {
      throw new CertificateServiceError(
        "No certificate template configured for this course's category",
        422
      );
    }

    const examAttempt = await certificateRepository.findExamAttemptById(
      examAttemptId
    );
    if (!examAttempt) {
    ...
    if (percentage < template.passThreshold) {
      throw new CertificateServiceError(
        `Score ${percentage.toFixed(2)}% is below the pass threshold of ${template.passThreshold}%`,
        422
      );
    }
    ...
    const expiresAt = computeExpiryDate(template.validityPeriod);
    ...
    const certificate = await certificateRepository.create({
      ...
      percentage,
      grade,
      awardCategory: template.awardCategory?.name ?? null,
      status: "Issued",
      issuedAt: new Date(),
      expiresAt,
      verificationUrl,
      ...
      categoryNameSnapshot: course.courseCategory.name,
